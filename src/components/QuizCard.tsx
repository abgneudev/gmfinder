...
export default function QuizCard({ gm, rank }: { gm: GM; rank: number }) {
  return (
    <Link
      ...
      className={`group relative flex flex-col rounded-2xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md ${
        RANK_RING[rank] ?? RANK_RING[2]
      }`}
    >
      <span className="absolute -top-3 left-5 inline-flex items-center gap-1 rounded-full bg-indigo-600 px-3 py-1 text-xs font-semibold text-white shadow-sm">
        {rank === 0 ? "✨ " : `#${rank + 1} · `}
      ...
      <h3 className="mt-3 font-semibold text-stone-900 group-hover:text-indigo-700">
        {gm.name}
      </h3>
